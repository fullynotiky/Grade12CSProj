...
class Level:
    ...
    def playerDeath(self):
        ...
        self.display.blit(self.player.finalScoreSurf, self.player.finalScoreRect)

        self.display.blit(self.startMenu.userNameSurf, self.startMenu.userNameRect.move(0, 130))
        self.display.blit(self.startMenu.highScoreSurf, self.startMenu.highScoreRect.move(0, 130))
    ...
    def run(self):
        ...
        if self.inGameStart:
            if self.inStartMenu:
                self.startMenu.displayStartMenu()

        elif self.gamePaused:
            self.upgrade.display()

        elif self.inSettingsMenu:
            self.display.blit(self.startMenu.transparentOverlay, (0, 0))
            self.ui.displaySettingsButton()
            self.startMenu.run()
            self.startMenu.displaySettingsMenu()
            self.displayScore()
